...
bool tcp_box_connection::readCommand()
{
  ...
  if(buffer == "uibkafc "){
      ...
      QByteArray response=m_box->getHelp_string().toLocal8Bit();
      simulate_delay();
      //write(response.toLocal8Bit());
      return (write(response) == response.size());
    } else {
      qDebug()<< Q_FUNC_INFO <<" command not supported or not implemented: "<<buffer;
      abort();
      return false;
    }
...
bool tcp_box_connection::parseSubCommand()
{
  ...
  if(buffer == "uibkafc getAll\r\n"){
      ...
      qDebug()<< Q_FUNC_INFO <<" subcommand not supported or not implemented: "<<buffer;
      abort();
      return false;
    }
