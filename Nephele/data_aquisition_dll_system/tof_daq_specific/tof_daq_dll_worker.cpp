...
namespace talorion{
        ...
        fatal_emited(false)
    {
        ...
        int rate =entity_manager_locator::get_instance()->get_updaterate_component(m_entity);
        timer->start(rate);

        connect(event_manager_locator::get_instance(),SIGNAL(updaterate_component_changed(int)),this,SLOT(updaterate_component_changed(int)));
        connect(this,SIGNAL(fatal(QString)),event_manager_locator::get_instance(),SIGNAL(fatal(QString)));
    ...
    {
        timer->stop();
        QString path= entity_manager_locator::get_instance()->get_user_data_path_component(m_entity);
        //        QString name;
    ...
    void tof_daq_dll_worker::register_user_data( int cmp_lvl)
    {
        ...
        QStringList value_names;
        foreach (int var, entity_manager_locator::get_instance()->get_entities_with_userdata_components()) {
            if(!registered_values.contains(var)){
                int component_id = entity_manager_locator::get_instance()->get_userdata_component(var);
                tmp = entity_manager_locator::get_instance()->getComponentDataForEntity(component_id,var);
                name =entity_manager_locator::get_instance()->get_name_component(var);
                if(name.isEmpty())
                    continue;
                if(tmp.isValid()){
                    tmp.toDouble(&ok);
