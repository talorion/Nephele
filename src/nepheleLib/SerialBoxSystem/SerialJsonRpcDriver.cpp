...
SerialJsonRpcDriver::SerialJsonRpcDriver(EntityManager::EntityID box, QObject *par) :
...
SerialJsonRpcDriver::~SerialJsonRpcDriver()
{
    m_pollTimer.stop();
...
bool SerialJsonRpcDriver::isDeviceConnected() const
{
...
SerialJsonRpcDriver::CommandQueue_t SerialJsonRpcDriver::cmdQ() const
{
    return m_cmdQ;
}

SerialJsonRpcDriver::MethodsList_t SerialJsonRpcDriver::methodNames() const
{
    return JsonRpcDriverUtils::getMethodNamesOfTcpBox(getBoxId());
}

bool SerialJsonRpcDriver::connectDevice()
{
...
void SerialJsonRpcDriver::disconectDevice()
{
...
void SerialJsonRpcDriver::sendMethod(EntityManager::EntityID box, const QString &methodname, const QVariant &params, qint32 id)
{
...
void SerialJsonRpcDriver::sendMethod(const QString &methodname, const QVariant &params, qint32 id, bool internalSoure)
{
    auto request = JsonRpcFactory::createRequest(methodname, params, id,internalSoure);
    sendMethod(request);
}

void SerialJsonRpcDriver::sendMethod(QSharedPointer<JsonRpcRequest> request)
{
    if(request.isNull())
        return;
...
void SerialJsonRpcDriver::sendDefaultRequest()
{
...
void SerialJsonRpcDriver::afterDeviceConnect()
{
    m_reconnectTimer.stop();
...
void SerialJsonRpcDriver::afterDeviceDisconnect()
{
    m_back.socketDisconnected();
    disconectDevice();
}

void SerialJsonRpcDriver::socketReadyRead()
{
...
void SerialJsonRpcDriver::pollQueue()
{
...
void SerialJsonRpcDriver::requestTimedOut(QString req_id)
{
    m_timoutErrorCounter++;
    m_consecutiveErrorCounter++;
...
bool SerialJsonRpcDriver::waitForDisconnected(int msecs)
{
    if(serialSocket() == Q_NULLPTR)
        return false;
...
void SerialJsonRpcDriver::sendJrpcRequest(QSharedPointer<JsonRpcRequest> request)
{
    if(isDeviceConnected() == false)
        return;
...
void SerialJsonRpcDriver::parseResponse(JsonRpcResponse *resp)
{
...
void SerialJsonRpcDriver::errorReport() const
{
    qDebug()<< Q_FUNC_INFO
