#ifndef TALORION_ARDUINO_BACKEND_HPP
#define TALORION_ARDUINO_BACKEND_HPP
...
namespace talorion {
    ...
    class arduino_backend :
            public abstract_backend

    {
        Q_OBJECT
    public:
        explicit arduino_backend(QObject *par = 0);
        ~arduino_backend(void);                                  // not desctructable
        Q_DISABLE_COPY(arduino_backend)
