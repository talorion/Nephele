...
//
        ...
        connect(this,SIGNAL(change_data_aquistition_dll_component(int,QString)),EventManager::get_instance(),SIGNAL(change_data_aquistition_dll_component(int,QString)));
        connect(this,SIGNAL(change_timeout_component(int,int)),EventManager::get_instance(),SIGNAL(change_timeout_component(int,int)));
        connect(this,SIGNAL(change_user_data_updaterate_component(int,int)),EventManager::get_instance(),SIGNAL(change_updaterate_component(int,int)));
        connect(this,SIGNAL(change_user_data_path_component(int,QString)),EventManager::get_instance(),SIGNAL(change_user_data_path_component(int,QString)));
...
//        timeout_field->setValue(to);
    ...
    {
        QList<int> dlls = EntityManager::get_instance()->get_all_TofDaqDlls();
        if(!dlls.isEmpty())
            current_entity =  dlls[0];
        else
            current_entity = EntityManager::get_instance()->createTofDaqDll();

        QString tmp = EntityManager::get_instance()->get_data_aquistion_dll_component(current_entity);
        dll_name_field->setText(tmp);

        int  to =  EntityManager::get_instance()->get_timeout_component(current_entity);
        timeout_field->setValue(to);

        int ur = EntityManager::get_instance()->get_updaterate_component(current_entity);
        user_data_updaterate_field->setValue(ur);

        QString udp = EntityManager::get_instance()->get_user_data_path_component(current_entity);
