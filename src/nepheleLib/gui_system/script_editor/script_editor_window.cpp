...
namespace talorion
{
        ...
        std_items()
    {
        ...
        connect(this,SIGNAL(debug_script_file(QString)),EventManager::get_instance(),SIGNAL(debug_script_file(QString)));

        connect(this,SIGNAL(change_script_file_component(int,QString)),EventManager::get_instance(),SIGNAL(change_script_file_component(int,QString)));

        connect(EventManager::get_instance(),SIGNAL(newQtScriptEngine(int)),this,SLOT(slot_newQtScriptEngine(int)));

        //QScriptEngineDebugger *m_debugger = new QScriptEngineDebugger(this);

        m_debugger = new QScriptEngineDebugger(this);

        QList<int> script_engines = EntityManager::get_instance()->get_all_Qt_Script_Engines();
    ...
    void script_editor_window::debug_script()
    {
        if (maybeSave()) {
            if(my_engine >= 0){
                QScriptEngine* eng =  EntityManager::get_instance()->get_qt_script_engine_component(my_engine);
    ...
    void script_editor_window::createActions()
    {
        ...
        connect(stopAct, SIGNAL(triggered()), EventManager::get_instance(),SIGNAL(abort_script()));

        skipSleepAct = new QAction(QIcon(":/images/images/skip.png"), tr("Skip Sleep"), this);
        skipSleepAct->setStatusTip(tr("Skip Sleep"));
        connect(skipSleepAct, SIGNAL(triggered()), EventManager::get_instance(),SIGNAL(script_skip_sleep()));
    ...
    void script_editor_window::init_engine(int entity)
    {
        ...
        QString tmp= EntityManager::get_instance()->get_script_file_component(entity);
        if(!tmp.isEmpty())
            loadFile(tmp);
