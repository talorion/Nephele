...
namespace talorion{
    ...
    TwToolDll::TwToolDll(QObject *par) :
        QObject(par),
    ...
    int TwToolDll::init(QString dll_name)
    {
    ...
    QString TwToolDll::translate_return_value(int ReturnValue) const
    {
    ...
    int TwToolDll::fit_single_peak(QVector<double>& yVals, QVector<double>& xVals, int peakType, double& blOffset, double& blSlope, double& amplitude, double& fwhmLo, double& fwhmHi, double& peakPos, double& mu) const
    {
        if(m_FitSinglePeak){
    ...
    int TwToolDll::fit_single_peak_2(QVector<double>& yVals, QVector<double>& xVals, int peakType, double& param)
    {
        if(m_FitSinglePeak2){
            int nbrDataPoints = qMin(yVals.length(), xVals.length());
            double *p_xVals = xVals.data();
    ...
    double TwToolDll::eval_single_peak(double xVal, QVector<double>& param) const
    {
        if(m_EvalSinglePeak){
    ...
    int TwToolDll::multi_peak_fit(QVector<double>& dataX, QVector<double>& dataY, QVector<double>& mass, QVector<double>& intensity, QVector<double>& commonPar, int options) const
    {
        if(m_MultiPeakFit){
    ...
    double TwToolDll::eval_multi_peak(double x,  QVector<double>& mass, QVector<double>& intensity, QVector<double>& commonPar) const
    {
        if(m_EvalMultiPeak){
            int nbrPeaks = mass.length();
    ...
    int TwToolDll::fit_resolution(QVector<double>& mass, QVector<double>& resolution, double &R0, double &m0, double &dm) const
    {
        if(m_FitResolution){
    ...
    double TwToolDll::eval_resolution(double R0, double m0, double dm, double mass) const
    {
        if(m_EvalResolution){
    ...
    //    QScriptValue TwToolDll::get_molecule_mass(QString molecule) const
    //    {
    ...
    int TwToolDll::get_molecule_mass(QString molecule, double &mass) const
    {
        if(m_GetMoleculeMass){
            QByteArray molecule_ba = molecule.toLocal8Bit();
    ...
    int TwToolDll::get_isotope_pattern(QString molecule, double abundanceLimit,  QVector<double>&isoMass, QVector<double>&isoAbundance) const
    {
    ...
    int TwToolDll::get_isotope_pattern_2(QString molecule, double abundanceLimit,  QVector<double>&isoMass, QVector<double>&isoAbundance) const
    {
    ...
    int TwToolDll::decompose_mass(double targetMass, double tolerance, QVector<double>&atomMass, QStringList atomLabel, int nbrFilters, int &elementIndex1, int &elementIndex2, double &filterMinVal, double &filterMaxVal, int &nbrCompomers) const
    {
        if(m_DecomposeMass){
    ...
    int TwToolDll::get_composition(int index, QString sumFormula, double &mass, double &massError) const
    {
        if(m_GetComposition){
    ...
    int TwToolDll::nist_library_search(int nbrSticks, QVector<double>&stickMass, QVector<double>&stickIntensity, bool interactive, unsigned int &nbrResults) const
    {
    ...
    int TwToolDll::nist_library_query_result(int index, int propertyIndex, QString& value) const
    {
    ...
    double TwToolDll::tof_to_mass(double tofSample, int massCalibMode, QVector<double>&p) const
    {
        if(m_Tof2Mass){
    ...
    double TwToolDll::mass_to_tof(double mass, int massCalibMode, QVector<double>&p) const
    {
        if(m_Mass2Tof){
    ...
    int TwToolDll::mass_calibrate(int massCalibMode,  QVector<double>&mass, QVector<double>&tof, QVector<double>&weight,  QVector<double>&p, QVector<double>&legacyA, QVector<double>&legacyB) const
    {
        if(m_MassCalibrate){
            int nbrPoints  = mass.length();
    ...
    int TwToolDll::brute_force_calibrate(QVector<double>&spectrum, int nbrPeaks, bool extendSearch, int triggerDelay, int pulseWidth, double sampleInterval, QVector<double>&a, QVector<double>&b) const
    {
    ...
    int TwToolDll::get_mass_calibInfo(int massCalibMode, QString& description, QVector<int>&nbrParams) const
    {
    ...
    int TwToolDll::enc_ims_correlate_profile(QVector<float>&profile, int opMode, QVector<int>&par) const
    {
    ...
    int TwToolDll::enc_ims_sharpen_profile(QVector<float>&profile, int opMode, QVector<int>&par, float sharpness) const
    {
    ...
    int TwToolDll::enc_ims_denoise_profile(QVector<float>&profile, int opMode, QVector<int>&par, float denoise, float sharpness) const
    {
    ...
    int TwToolDll::enc_ims_correlate_multi_profiles(void *data, int dataType, int nbrProfiles, int calcMethod, float correlateThreshold, int opMode, int *par) const
    {
    ...
    int TwToolDll::enc_ims_cleanup() const
    {
        if(m_EncImsCleanup){
    ...
    int TwToolDll::si_initialize_histograms( QVector<double> &loMass, QVector<double> &hiMass, QVector<int> &specType) const
    {
    ...
    int TwToolDll::si_set_processing_options(QString option, double value, int specType) const
    {
    ...
    int TwToolDll::si_process_spectrum(QVector<float> &spectrum,  int specType, float &blFromData, float &thrFromData) const
    {
    ...
    int TwToolDll::si_get_histogram(int histogramIndex, QVector<float> &intensity, QVector<unsigned int> &counts,  unsigned int &spectrumCount, double &meanValue) const
    {
        if(m_SiGetHistogram){
    ...
    int TwToolDll::si_get_sum_histogram(int specType, QVector<float> &intensity, QVector<unsigned int> &counts,  unsigned int &spectrumCount, double &meanValue, double minMass, double maxMass, double minRate, double maxRate) const
    {
    ...
    int TwToolDll::si_reset_histograms() const
    {
    ...
    int TwToolDll::si_cleanup() const
    {
        if(m_SiCleanup){
            return (m_SiCleanup());
        }
    ...
    int TwToolDll::si_fit_phd(QVector<double>&intensity, QVector<double>&counts, double *fwhm, double *a, QVector<double>&par) const
    {
        if(m_SiFitPhd){
            int nbrPoints = 0;
            if(par.length()<4)
    ...
    double TwToolDll::si_eval_phd(QVector<double>&par, double intensity) const
    {
        if(m_SiEvalPhd){
            if(par.length()<4)
                par.resize(4);
    ...
    int TwToolDll::si_fit_rate_from_phd(int nbrPoints, double *intensity, double *counts, double *siPar, double *rate, double *fitCounts, int *nbrNIonTraces, double **nIonTrace) const
    {
    ...
    int TwToolDll::find_tps_ip(QString tpsSerial, int timeout,  QString &hostStr) const
    {
        if(m_FindTpsIp){
