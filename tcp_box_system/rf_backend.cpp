...
namespace talorion {
    ...
    {
        double value =entity_manager_locator::get_instance()->get_analogSetValue_component(entity);
        int id = entity_manager_locator::get_instance()->get_id_component(entity);
    ...
    void rf_backend::dvSetChangeProxy(int entity)
    {
        ...
        bool valueb =entity_manager_locator::get_instance()->get_digitalSetValue_component(entity);
        int id = entity_manager_locator::get_instance()->get_id_component(entity);
        if(id >= 0 && registered_values.contains(id)){
            if(registered_values[id]!=entity)
                return;
    ...
    void rf_backend::parse_alldata_AO(QVariantList &desc, int box_id)
    {
        foreach (QVariant var, desc) {
            ...
            {
                int fc = entity_manager_locator::get_instance()->createNewAnalogOutputValue(tmp.find("name").value().toString(),
    ...
    void rf_backend::parse_alldata_AI(QVariantList &desc, int box_id)
    {
        foreach (QVariant var, desc) {
            ...
            {
                int fc = entity_manager_locator::get_instance()->createNewAnalogInputValue(tmp.find("name").value().toString(),
                                                                                   tmp.find("units").value().toString(),
    ...
    void rf_backend::parse_alldata_AIO(QVariantList &desc, int box_id)
    {
        foreach (QVariant var, desc) {
            ...
            {
                int fc = entity_manager_locator::get_instance()->createNewAnalogValue(tmp.find("name").value().toString(),
                                                                              tmp.find("units").value().toString(),
                                                                              tmp.find("smin").value().toDouble(),
                                                                              tmp.find("smax").value().toDouble(),
                                                                              tmp.find("amin").value().toDouble(),
    ...
    void rf_backend::parse_alldata_DO(QVariantList &desc, int box_id)
    {
        foreach (QVariant var, desc) {
            ...
            {
                int fc = entity_manager_locator::get_instance()->createNewDigitalOutputValue(tmp.find("name").value().toString(),
                                                                                     tmp.find("set").value().toDouble(),
                                                                                     tmp.find("id").value().toInt(),
                                                                                     box_id
                                                                                     );
