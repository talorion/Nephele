...
namespace talorion {
        ...
        abstract_scriptable_object("set", par)
    {
        ...
        connect(this,SIGNAL(set_dig_value_changed(int,bool)),event_manager::get_instance(),SIGNAL(change_digitalSet_component(int,bool)),Qt::UniqueConnection);
        connect(event_manager::get_instance(),SIGNAL(analogSet_component_changed(int)),this,SLOT(slot_set_value_changed(int)));
        connect(event_manager::get_instance(),SIGNAL(digitalSet_component_changed(int)),this,SLOT(slot_digital_set_value_changed(int)));
        connect(event_manager::get_instance(),SIGNAL(newAnalogValue(int)),this,SLOT(slot_newAnalogValue(int)));
        connect(event_manager::get_instance(),SIGNAL(newAnalogOutputValue(int)),this,SLOT(slot_newAnalogValue(int)));
        connect(event_manager::get_instance(),SIGNAL(newDigitalValue(int)),this,SLOT(slot_newDigitalValue(int)));
        connect(event_manager::get_instance(),SIGNAL(newDigitalOutputValue(int)),this,SLOT(slot_newDigitalValue(int)));
    }
    ...
    void script_set_handler::val(QString name, double value)
    {
        ...
        if(!entities.isEmpty()){
            ...
            if(entity_manager::get_instance()->is_analog_value(entity))
                emit set_value_changed(entity, value);
            if(entity_manager::get_instance()->is_digital_value(entity))
                val(name, value != 0);
        }
    }
    ...
    {
        QString nme = entity_manager::get_instance()->get_name_component(entity);
        bool valu = entity_manager::get_instance()->get_digitalSetValue_component(entity);
        if(getScrip_value() != NULL)
            getScrip_value()->setProperty(nme, valu, QScriptValue::ReadOnly);
    }
